---
id: 01KZ321GMJ8VYSCFE7FT5RXXTM
number: 517444800
type: artifact
schema: artifact/v1.1
name: Slice 03 ledger -- native authoring commands (programmatic surface)
created: 2026-08-02
updated: 2026-08-03
origin: planned
reserved: false
source:
  paths:
  - docs/design-v1.0.0/arc-store-as-source/slice03-native-authoring/ledger.md
  class: ledger
...
| F-7 | **SS-4 demonstrated**: a node body is **created AND updated** entirely via `./bin/odm`, no hand-edit of store files; round-trips through `node show` | e2e: new (metadata+content) -> set -> set-body -> show, no hand-edit | serious | arc SS-4 | done (fixture); real leg deferred | `cli.rs::ss4_round_trip_new_set_set_body_show_no_hand_edit` — `node new` (metadata+content) → `node set` (status) → `node set-body` (replace) → `node show --json`, `check` exit 0 after **every** step, no file ever hand-edited. Real-corpus leg (the identical round-trip against `.worktrees/odm`) not run — see F-3's Notes / the standing blocker every slice this session has hit. | |
