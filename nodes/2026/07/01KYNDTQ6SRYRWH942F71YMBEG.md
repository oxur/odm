---
id: 01KYNDTQ6SRYRWH942F71YMBEG
number: 61823100
type: arc
schema: arc/v1.1
name: 'Arc — Release Hardening: CLI, types, naming & output (UAT-driven) — plan-of-record'
created: 2026-07-28
updated: 2026-07-28
origin: planned
reserved: false
source:
  paths:
  - /Users/oubiwann/lab/oxur/odm/.worktrees/1.0.x/docs/design-v1.0.0/arc-release-hardening/arc-plan.md
  class: arc-plan
...
| **C-6 — `check` hardening** | Three check-rule additions in **one pass** over `check` (churn once): **G-2** — persist tear-rationale + surface it (`tear --because` validates but the schema drops it → a data-loss bug on a *phantom* slice route; add the field); **G-3** — decomposed/orphan rule (a parent with children but no `decomposed` assertion, or an assertion that disagrees with reverse-`part_of` → warn; `--strict` errors); **L-3b** — a project with **no `# Vision` body** is a finding (the DoD depends on it; sibling to L-3a's data fix).  **DONE 2026-07-27** (`c6-closing-report.md`): **G-2 was already implemented** — `TornEdge.because` exists, round-trips, and `validate` lists tears with rationale; ODD-0013 §4.3 already documents it, so no schema amendment was needed. Real gap closed: `node show` now lists tears + rationale (human and `--json`). The rationale-less warn **declined** — it would require making `because` optional, weakening a parse-time guarantee into a runtime nag (pinned by test). **G-3** shipped warn-by-default (`--strict` promotes), exclusive with `advanced-without-decomposition`; blast radius on odm's corpus is **2 nodes** (#1000, #1600), not the whole corpus — arcs 1100–1500 already carry assertions. **L-3b** shipped. | model (G-2 schema field) + check rules | **G-2, G-3, L-3b** | — |
...
| RH-6 | **Compose:** the self-hosted CLI is coherent + themed + UAT-validated end-to-end | arc-scale demo: run `odm list`/`orient`/`project`/`chain` on the real corpus — coloured, tree-structured, renamed, de-numbered | serious | arc-plan / UAT | **reproduced** | Reproduced by CDC on the self-hosted store at `release/1.0.x` @ `839fa26`, and re-run independently by CC at `ba6aa45` while writing the arc closing report — the whole RH surface in one pass, every chunk's work visible together: `orient` renders VISION (L-3a) + CURRENT FOCUS #1600 (C-5) + READY `design` nodes (C-2) + INTEGRITY ok; `project`/`chain` resolve (C-4 renames); `validate` exit 0 with the two G-3 warns (C-6) and `check` exit 0 (C-4's split); `odm node list` (C-4 group) renders the C-1 theme, C-3 tree + de-numbered names, C-5 real dates and clean names, C-2 types and C-8 normalized STATUS. |
| RH-7 | **Compose:** re-running `odm self-host` after the type/name changes yields a `check`-green corpus with `design`/`research` types + de-numbered names | arc-scale demo: re-self-host → `odm check` green; nodes carry `design/v1.0` etc. | serious | arc-plan / ODD-0013/0020 | **reproduced** | Attested via the C-5 cutover and standing green since: odm's own corpus re-derived into the store home, `validate` exit 0 at **60 nodes**, `design`/`research` types, de-numbered names, `schema: <type>/v1.0` markers intact. **Nuance on the record:** the corpus is **re-stamped in place, not minted fresh** — a truly fresh re-derivation would mint new ULIDs, break every edge and trip the G-1 freeze, so the loop validates the *derivation logic* via the re-stamp. That is the strongest form available until G-1 is decided.  | reproduce at arc scale. The reflexive validation loop. **Nuance for the record:** the corpus is **re-stamped in place**, not minted fresh — so this validates the *derivation logic* via the re-stamp. A truly fresh re-derivation (new ids) is **out of scope pending G-1**, and would in fact break every edge; C-5 preserved all 60 ULIDs deliberately. |
| RH-8 | bubble-up findings dispositioned | ptr: arc-plan change-log | correctness | bubble-up | **done** | | accrues as chunks close. **C-1 (v1.2):** F-1 dispositioned (Route B / `oxur-term`); F-15 surfaced while validating C-1's themed `list` (routed to C-3, not a C-1 defect) — logged, not dropped. **v1.3:** F-16 (unconditional table ANSI) re-seated from a simultaneous-write id collision — logged, unassigned. **C-2 (v1.6):** F-2/F-3 dispositioned; **F-17** (body-snapshot drift) logged — a migrate/self-host decision, not C-3. **C-3 (v1.7):** F-4…F-9 + F-15 dispositioned; the F-15 audit corrected the "~5 superseded ODDs" to **zero** (prose matches, not markers) — recorded rather than quietly dropped. **C-3 CDC (v1.8):** independently reproduced — 1 retired (#1605), 0 active `supersedes` edges (the ~5 was prose). **C-3 review → F-18:** work-node names embed doc metadata (`(plan-of-record)` suffix, 32/47) — routed to a new **C-7** (source name-normalization + §2.1 rule); C-6 stays reserved for G-2. **C-3 review → F-19:** STATUS (furthest gate) isn't comparable across types → new **C-8**, a display-only normalized `not-started/in-progress/done` state. **C-3 review → F-20/F-21 (v1.11):** `self-host` discards the plan's real creation dates (data loss → **C-5**); `--json` omits `created`/`updated` (unassigned). Both surfaced by reading C-3's DATE column; neither is a C-3 defect. **C-6 (v1.15):** G-2/G-3/L-3b dispositioned. **Two of the chunk's factual premises were wrong and are corrected here rather than worked around.** G-2 described the tear rationale as a data-loss bug with no schema field: the field exists, round-trips, is listed by `validate`/`--json`, and ODD-0013 §4.3 already documents it — so Phase 0's amendment had nothing to amend, and the only real gap was `node show` not mentioning tears (now fixed). G-3's heads-up predicted odm's corpus carried *no* `decomposed` assertions and would light up: arcs 1100–1500 all carry them, so the gap is exactly 2 nodes. **One sub-decision declined with reason:** warning on a rationale-less tear would require making `because` optional, trading a parse-time impossibility for a runtime nag — the stronger guarantee is kept and pinned by a test. **One dynamic recorded, not resolved:** G-3 warns on parents still mid-decomposition (#1600), and affirming early is what `decomposition-drift` — an *error* — catches later if the child set grows; warn-by-default was chosen with that trade-off on the table. **C-8 (v1.14):** F-19 dispositioned **done**. Labels are **`planned`/`active`/`done`**; `--status` accepts the derived *and* raw vocabularies so the flag and the column cannot disagree while F-15's raw spellings keep working; `blocked` deferred (needs the graph); `--status-format=gate` not built. **One deviation, and the important one:** the chunk's case for a normalized-only column was that `show`/`--json` already carried the raw ladder — they did not carry it at all, so the rung would have become unreachable from the CLI. Fixed in-chunk by giving `show` a gate ladder and `--json` a `status` + `gates` pair, which is what the brief's own acceptance text already assumed. The per-gate colour palette retired with the raw column (nothing could select it); its slots live on, and the `complete`-green / `verified`-bright-green distinction now survives only in `show`. **C-4 (v1.13):** F-10…F-13 dispositioned **done**, folded with **ODD-0023 → Accepted (v1.1)**. Three of the ODD's tentative calls were **reversed** and the reversals recorded rather than quietly rewritten: `use` kept (C-5 gave it a workflow the draft could not have known about), `validate`/`check` inverted so the pure operation owns the honest name, and a hard cut instead of a deprecation window. **Three parity defects surfaced by the inventory↔`--help` check** — a documented `store sync` command that was never built, `store init` help stale since slice 03, and the now-dead `self-host` CLI wrapper — none of them a C-4 defect, all fixed here because the chunk owns surface parity. Parity is now verified mechanically (11/11/2) rather than by reading. **C-5 (v1.12):** F-14/F-18/F-20/L-3a dispositioned **done**, and **SH-6 closed jointly with arc-store-home** — the cutover. C-7 formally retires into C-5. Three new findings, all from the dogfood itself and all fixed in-chunk: **(a)** `orient` resolved the CLI context against the *invocation* root while `use`/`context` used the *store* root — invisible while the two were the same directory, and the reason `use` could print success and `orient` still report "(no current arc)"; fixed by deriving the path from the `Store` handle so no caller can pass the wrong root. **(b)** `store init` scaffolded no `.gitignore`, so a store branch offered its derived index for commit. **(c)** the first fix for (b) ignored all of `.odm/`, which would have withheld `context.json` — the current focus — from every fresh clone, defeating the project's stated success test that a fresh session orients from `odm orient` alone; the rule is now ignore-the-directory-plus-one-exception, because a list of caches fails open (`.odm/drift` was already missed). **Two brief corrections recorded, not silently absorbed:** the re-stamp relocates nothing (the month shard is a function of the ULID, not the date), and document node #15's `(build plan)` is its real H1 title rather than importer metadata, so it stays. **Arc close (v1.16):** all 21 F-rows dispositioned — 18 done, **F-16** carried (the `NO_COLOR`/TTY guard belongs upstream in `oxur-term`, not as a local rule), **F-17** carried as a decision not a defect (snapshot-by-design vs refresh-on-re-migrate), **F-21** routed to the LLM arc as a `--json` contract change. Routed L/G rows closed out: L-3a/L-3b/L-1/L-9 and G-2/G-3/G-4 done, **L-8b (reconcile 4 ODDs) still a pre-ship gate**, **G-1 still gates minting any new node** — unchanged by this arc, and C-5 was built to preserve every ULID precisely so it would not trip it. See `closing-report.md`. |
| RH-9 | C-8 (normalized display status) closed | ptr: C-8 `cdc-verification.md` (+ `c8-closing-report.md`) | serious | arc-plan | **attested** | `c8-closing-report.md` (2026-07-27): build/test (**58 binaries, 0 failed**)/clippy `-D warnings`/fmt green; no `unsafe`. STATUS shows a node's position in its own ladder — **`planned`/`active`/`done`** plus the withdrawal overlays — so a done arc (`verified`) and a done slice (`tested`) read alike and an arc at `complete` reads `active`. Pure render-time derivation: `nodes/**` untouched, `validate` green at 60, counts unchanged. `--status` accepts the derived **and** raw vocabularies (50 rows for `--status done`, 0 of them showing anything else). | attested-by-CC → **reproduced** on CI. **The brief's premise for a normalized-only column was false and was fixed in-chunk:** it argued `show`/`--json` already carried the raw ladder — neither reported gates at all, so normalizing alone would have made the rung unreachable from the CLI. `node show` now prints the state plus every rung reached/unreached; `--json` carries `status` + `gates`. Two unspecified cases decided against borrowing meaning: a type with **no** ladder (an `adr`) reports `—`, not `planned`, and `--json` omits the field rather than emitting the display em-dash. |
| RH-10 | C-6 (`validate` hardening: G-2 + G-3 + L-3b) closed | ptr: C-6 `cdc-verification.md` (+ `c6-closing-report.md`) | serious | arc-plan | **attested** | `c6-closing-report.md` (2026-07-27): build/test (**58 binaries, 0 failed**)/clippy `-D warnings`/fmt green; no `unsafe`. **G-3:** the affirmation requirement broadens from "before you call it done" to "once you have decomposed at all" — `undecomposed-parent`, warn by default, `--strict` promotes to exit 1; exclusive with the existing `advanced-without-decomposition` so one gap is never counted twice. **L-3b:** a project with no `# Vision` section is reported (`no-vision`), via a targeted body load since bodies are out of the index; any heading level/case counts, an empty section does not. Both are static, so `check` inherits them — asserted, not assumed. | attested-by-CC → **reproduced** on CI. **G-2 was already done:** the brief called the tear rationale a data-loss bug, but `TornEdge.because` exists, round-trips, and is listed by `validate`/`--json`, and ODD-0013 §4.3 already documents it as required-and-never-dropped — so **Phase 0's schema amendment had nothing to amend**. The one real gap was `node show` never mentioning tears; it now lists them with rationale, in human and `--json`. **The rationale-less warn was declined with reason:** it would require making `because` optional, trading a parse-time impossibility for a runtime nag — the guarantee is now pinned by a test instead. **The brief's G-3 heads-up was also wrong** (it predicted odm's corpus had *no* `decomposed` assertions; arcs 1100–1500 all carry them), so the real blast radius is 2 nodes — #1000 and #1600 — predicted from the corpus before the code and confirmed after. Recorded dynamic: #1600 is mid-flight, and affirming a decomposition early is what `decomposition-drift` (an **error**) catches later if a slice is added, so the rule nudges toward an assertion that can become a defect; flagged at kickoff, warn-by-default chosen deliberately. |
