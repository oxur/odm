...
use odm::doc::{build_yaml_frontmatter, DesignDoc, DocMetadata, DocState};
use odm::filename::slugify;
use odm::git;
...
fn extract_basic_doc(content: &str, path: &PathBuf) -> Result<DesignDoc> {
    let title = odm::doc::extract_title_from_content(
        content,
...
fn merge_metadata(
    ...
    let version = if let Some(override_ver) = version_override {
        ...
        if let Ok(is_valid) = odm::doc::is_version_valid_upgrade(&old_meta.version, &override_ver) {
            if !is_valid {
        ...
        match odm::doc::increment_minor_version(&old_meta.version) {
            Ok(new_ver) => new_ver,
            Err(_) => {
