...
pub fn add_document(
    ...
    if let Err(e) = odm::git::git_add(&final_path) {
    ...
    println!();
    let index = odm::index::DocumentIndex::from_state(state_mgr.state(), state_mgr.docs_dir())?;
...
fn determine_author_auto(extracted: &ExtractedMetadata) -> String {
    extracted.author.clone().unwrap_or_else(|| odm::git::get_author(std::path::Path::new(".")))
}
...
mod tests {
    use super::*;
    use odm::extract::ExtractedMetadata;
    use std::fs;
    ...
    fn test_add_document_invalid_markdown() {
        ...
        let result =
            add_document(&mut state_mgr, file_path.to_str().unwrap(), None, false, false, true);
        assert!(result.is_err());
    ...
    fn test_determine_title_interactive_would_use_defaults() {
        ...
        let default_title = extracted
            ...
            .or(extracted.first_heading.as_ref())
            .cloned()
            .unwrap_or_else(|| odm::filename::filename_to_title("file-name.md"));
