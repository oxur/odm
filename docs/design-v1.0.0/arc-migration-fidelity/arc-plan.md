...
>
> **Status:** s01–s03 **closed** (2026-07-27); s03 **CDC-verified PASS**; **s04 (scope + repair
> *capability*) next**. s04 splits into a fixture-proven capability, then **s05 the live repair
> run** (operator call 2026-07-27 — don't build a destructive op and fire it in the same slice; the
> `MAX_MVP_ARC` cap is removed, see v1.6). *Plan late, plan deep* — each slice's open set is written
> when it becomes active, and its sizing is confirmed then.
...
| **s04 — scope + repair capability** | **Remove the `MAX_MVP_ARC` cap** (`self_host` imports **all** arc/slice dirs; named arcs get a non-structural `number` handle; `coverage.rs` shares `arc_in_scope`). **Update-in-place repair** of stub nodes (ODD-0025 §2.8 — real body + `source`, preserve id/edges/status, via `persist` overwrite — **no `delete` needed**). The **ODD-0020 schema-minor bump** (`v1.0→v1.1`). **Fixture-verified; no live mutation.** | no (fixture-only) | s03 |
...
| MF-2 | Body-hash gate green across all migrated nodes; zero stub bodies remain | re-migrate + gate; count stubs = 0 | serious | planned — s03 built + fixture-verified the gate (CDC-verified); s04 builds the repair capability; **s05's live run** produces "zero stubs" |
| MF-3 | Every migrated node carries a `source` sub-map (+ preserved `author`/`version`) | grep/`check` over the store | correctness | planned — s03 typed + populates the fields (fixture, CDC-verified); the live 60-node corpus gets them in **s05** |
...
| MF-5 | All arcs (incl. the 6 previously-excluded) + all slices represented | count dirs vs nodes = 0 gap | serious | planned — s04 removes the cap + assigns named-arc handles (capability); **s05** mints the missing arcs/slices live |
