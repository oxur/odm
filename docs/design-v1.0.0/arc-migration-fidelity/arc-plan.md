...
| **s14 — config-driven migrate roots** ⏳ OPEN 2026-07-31 (CDC-drawn; awaiting CC) | **Inserted before the arc-close freeze** — the operator surfaced three defects in the shipped `migrate --all`: (1) the required `<LEGACY_PATH>` positional is wrong (migrate should run from config, positional carries only *additional* dirs); (2) the legacy `docs_directory` + `"design"` **append was dropped** — `all()` uses `configured_docs_directory` as-is, which reconciles design/research over all of `./docs` once the config moves to the canonical `[legacy] docs_directory = "./docs"`; (3) no persistent additional-paths for arbitrary legacy dirs (research/brainstorm/chats). s14 makes migrate **config-driven**: drop the positional, add an optional comma-separated `[ADDITIONAL_PATHS]`, restore `docs_directory/design`, add `[legacy].additional_paths` (sorted+uniqued, union-on-write, re-read each run), and **set-subtract** overlaps so a design/dev dir is never re-processed by the generic pass. Also resolves the config-home subtlety (the code reads the store `config.toml` via `operational_text()`, not the code-branch `odm.toml` `[legacy]` the operator edited). **Capability/fixture only — `.worktrees/odm` untouched**; the corrected `migrate --all` is fired live by the arc-close freeze (the s12→s13 rhythm again). `slice14-migrate-config-roots/{slice-doc,ledger,cc-prompt}.md`. | no (fixture-only) | s13 |
