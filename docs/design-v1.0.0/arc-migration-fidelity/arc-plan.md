...
> add the `mint_artifacts` `is_excluded` guard), so **s15 blocks the freeze**. Sequence: **s15 → freeze
...
| **s15 — reconcile completeness** ⏳ OPEN 2026-08-01 (CDC-drawn; awaiting CC) | **Inserted before the arc-close freeze** — the config-fixed final-reconcile dry-run (2026-07-31) reconciled 2 of 4 drifted nodes and **structurally could not** close the other 2: `#1001` (the 1:1 `project-plan` node) is orphaned by `reconcile_source`'s blanket `node_type == Project` exclusion (selfhost.rs:578) + the live-reconcile loop's `matches!(Arc\|Slice)` filter (selfhost.rs:757) — the exclusion meant to protect the synthesis `#1000` also excludes the 1:1 base whose whole purpose is fidelity; and `#509907700` (the slice10 `ledger.md` artifact) is skipped because `mint_artifacts` only mints *uncovered* docs, never re-snapshotting a drifted covered one. s15 (capability/fixture) narrows the exclusion to key on **`source.synthesis`** (protect `#1000`, reconcile `#1001`; ODD-0020 v1.4 consistency), makes `mint_artifacts` **mint-or-reconcile** (D-1), and folds in the `mint_artifacts` `is_excluded` guard (so `index.md`/`templates/*` can't be minted regardless of root — the latent fragility found while diagnosing the s14 config). Two decisions flagged: D-1 (artifacts reconcile vs snapshot-scope-out) and D-2 (reconcile locus: self-host vs `apply_project_vision`). **Capability/fixture only — `.worktrees/odm` untouched**; the completed reconcile is fired live by the arc-close freeze. `slice15-reconcile-completeness/{slice-doc,ledger,cc-prompt}.md`. | no (fixture-only) | s13 |
