...
- Build & run the **doc-coverage detector** (320 sources ↔ 60 nodes) — the first real hole count. NOTE: until provenance exists, coverage is **heuristic** (title/number/type); provenance makes it an exact set-difference. Say so in the numbers.
