...
| *(added in review)* TYPE palette | One hue per node type, so a long listing sorts by kind without being read: `project` magenta, `arc` violet, `slice` blue, `design` yellow, `research` red-orange. Truecolor rather than the basic ANSI slots — violet and orange have no basic slot, and the values are tuned to stay legible on the theme's `#451A03` band. `adr`/`note` are left uncoloured rather than being assigned a hue by omission. |
