...
| **A2 — Graph, gates & derived order** | Typed edges; petgraph DAG; cycles + explicit tears; multi-gate status vectors **with evidence-level**; satisfaction; `next`/`blocked`/`path`/topo; staleness guard; decomposition/recomposition integrity (+ `decomposed: complete`); `check` v2. | A1 | odm-graph, odm-core |
