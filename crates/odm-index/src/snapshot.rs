...
/// `origin` + `decomposed`. **v4** (RH C-3): `created` + `retired`, which
/// `list` needs to lead with the creation date and to exclude retired nodes
/// without re-parsing the corpus. An older index loads as
